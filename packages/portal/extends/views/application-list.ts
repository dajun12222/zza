...
export default class AppList2 extends Vue {
  ...
  isGetListConfigDataIng: boolean = false;
  ...
  async getListConfigData(param?: any) {
    if (this.isGetListConfigDataIng) {
      return;
    }
    ...
    this.isGetListConfigDataIng = true;

    const res = await listApi.getListConfigData(params);
    this.isGetListConfigDataIng = false;
    // 如果获取数据失败
        ...
        queryConditions.forEach((item: any) => {
          try {
            ...
            if (dictionaryData && dictionaryData.checkedDictionary) {
              ...
              let resOptions = dictionaryData.useDictionariesData.map(
                (el: any) => el.name,
              );
