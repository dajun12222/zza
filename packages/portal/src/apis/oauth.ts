...
//获取验证码
const verificationUrl: string = `${env.oauthHost}/login/Authentication/get_verification_code`;
...
export default {
  ...
  //获取tab
  getAuditTab(code?:string){
    return Axios.get(`/api/audit/public/getAuditTab?code=${code}`)
  },
  //获取新闻
  getAuditFocus(code?:string){ 
    return Axios.get(`/api/audit/public/getAuditFocus?code=${code}`) 
  }
};
