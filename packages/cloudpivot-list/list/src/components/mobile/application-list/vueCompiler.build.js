...
let deindent = _interopDefault(require('de-indent'));
 ...
 */
let hasOwnPropertyFunc = Object.prototype.hasOwnProperty;
function hasOwn(obj, key) {
  return hasOwnPropertyFunc.call(obj, key);
}
...
if (typeof Set !== 'undefined' && isNative(Set)) {
  ...
  _Set = /*@__PURE__*/ (function () {
    ...
    SetFunc.prototype.has = function has(key) {
      return this.set[key] === true;
    };
    SetFunc.prototype.add = function add(key) {
      this.set[key] = true;
    };
    SetFunc.prototype.clear = function clear() {
      this.set = Object.create(null);
    };
...
strats.watch = function (parentVal, childVal, vm, key) {
  ...
  for (let key$1 in childVal) {
    if (childVal.hasOwnProperty(key$1)) {
...
function parseFor(exp) {
  ...
  const alias = inMatch[1].trim().replace(/^\(/, '').replace(/\)$/, '');
...
let createCompiler$1 = createCompilerCreator(function baseCompile(
  template,
  options,
) {
  const ast = parse(template.trim(), options);
...
let ref$1 = createCompiler$1(baseOptions);
let compile$1 = ref$1.compile;
let compileToFunctions$1 = ref$1.compileToFunctions;
