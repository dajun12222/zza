...
<template>
  <div class="search">
    <div class="search-header">
      <div
        v-show="schemaCode === 'onlineLearning'"
        :class="[
          'search-header-inner',
          learn === true && 'selectActive',
        ]"
        @click="switchClass"
      >
        <span>课程分类</span>
        <i
          :class="[
            'icon aufontAll',
            learn === false && 'h-icon-all-down-o',
            learn === true && 'h-icon-all-up-o',
          ]"
        ></i>
      </div>
      <div
        ...
        :key="index"
        :class="[
          schemaCode !== 'onlineLearning' && 'search-header-inner',
      ...
      <div v-if="canDelete && schemaCode !== 'onlineLearning' && schemaCode !== 'onlineExam' && schemaCode !== 'onlineCourseExam'">
    ...
    <search-bar
        ...
        @cancel="cancelCourse"
      />
    <form-list-sort v-show="schemaCode !== 'onlineLearning'" :querySorts="querySorts" @sort="handleSort" />
    ...
    <div v-show="learn" class="searchClass">
      <div class="classMenu">
        <div v-for="(item,index) of searchClassList" 
          :key="item.id" 
          :class="[
            index === classIndex && 'selectClassActive',
          ]"
      ...
      <div class="classMenuCon">
        <div class="classMenuConL">
          <div 
            v-for="(items,i) of searchClassList[classIndex].children" 
            :key="items.id"
            :class="[
              i === classMenuIndex && 'classActive',
            ]"
            @click="handleClassMenu(i)"
          >
            {{ items.name }}
          </div>
        </div>
        <div class="classMenuConL">
          <template v-if="classIndex !== 0 && classMenuIndex !== '' && searchClassList[classIndex].children[classMenuIndex].children">
            <div 
              v-for="(menuItem, menuIndex) of searchClassList[classIndex].children[classMenuIndex].children" 
              :key="menuItem.id"
              :class="[
                menuIndex === classItemIndex && 'classActive',
              ]"
              @click="handleClassMenuItem(menuIndex)"
            >
              {{ menuItem.name }}
            </div>
          </template>
...
export default class ListFormSearch extends Vue {
  ...
  learn: any = false;  //课程分类筛选

  searchCourseName:any = false;  //搜索知识
  ...
  switchCourseName(){
    this.searchCourseName = !this.searchCourseName;
  }
  ...
  handleClass(index){
    ...
    this.classItemIndex = '';
    if(index === 0){
      this.$emit('knowledgeType','');
      this.learn = false;
    }
  }

  handleClassMenu(index){
    this.classMenuIndex = index;
    this.classItemIndex = '';
    if(this.classIndex !== 0 && this.classMenuIndex !== '' && this.searchClassList[this.classIndex]['children'][index]['children']){

    }else{
      let val:any = ''
        val = this.searchClassList[this.classIndex].name+'/';
        val += this.searchClassList[this.classIndex]['children'][index].name;
        this.$emit('knowledgeType', val);
       this.learn = false;
    }
  }

  handleClassMenuItem(index){
    this.classItemIndex = index;
     let val:any = ''
        val = this.searchClassList[this.classIndex].name+'/';
        val += this.searchClassList[this.classIndex]['children'][this.classMenuIndex].name+"/";
        val += this.searchClassList[this.classIndex]['children'][this.classMenuIndex]['children'][index].name;
    this.$emit('knowledgeType', val);
     this.learn = false;
  }

  searchCourse(val:any){
    this.$emit('courseName',val);
  }

  cancelCourse(){
    this.$emit('courseName','');
  }
  ...
  initPage() {
    ...
    if(
        this.schemaCode === 'onlineExam' ||
        this.schemaCode === 'onlineLearning' || 
        this.schemaCode === 'onlineCourseExam'
    ){
      this.searchWays = [
        {
          name: `${this.$t('cloudpivot.list.mobile.Filter')}`,
  ...
  async searchDictionaryRecord(){
    await axios.get(
      'http://192.168.0.16/api/api/data_dictionary/searchDictionaryRecord?id=2c2c808386cb5a900186defa7e23176a&searchWord=&page=1&size=99999'
      ).then((res) => {
      if(res['errcode'] === 0){
        this.searchClassList = this.searchClassList.concat(res.data.content);
      }
    });
  }
...
.search {
  ...
  .classMenu{
    ...
    .selectClassActive{
      background: #fff;
      border-left:2px solid rgba(77,129,199);
      color: rgba(77,128,193);
    }
  }
  .classMenuCon{
    width: 60%;
