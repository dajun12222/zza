...
<template>
  <a-modal
    ...
    :title="$t('cloudpivot.list.pc.importErrEdit.title')"
    width="80%"
  ...
  >
    <div v-if="isHalfSuccess" class="data-import-status-half">
      <p class="data-import-status-half__success">
        ...
        <span>{{
          $t('cloudpivot.list.pc.importErrEdit.successTips', {
            num: successNum,
          })
        }}</span>
      </p>
      <p class="data-import-status-half__error">
        ...
        <span v-if="failNum && failNum > 0">{{
            $t('cloudpivot.list.pc.importErrEdit.errorTips2', { num: failNum })
          }}
          <a href="javascript:;" @click="exportErrorResult">{{
            $t('cloudpivot.list.pc.importErrEdit.errorTips5')
          }}</a>{{ $t('cloudpivot.list.pc.importErrEdit.errorTips3') }}
        </span>
        <span v-else>{{
          $t('cloudpivot.list.pc.importErrEdit.errorTips', { num: failNum })
        }}</span>
      </p>
    </div>
    <div class="line"></div>
    <div class="title-flex">
      <span v-if="hasErrorStatus">
        ...
        {{ $t('cloudpivot.list.pc.importErrEdit.errorTips4') }}
      </span>
      <span v-else class="success">
        <i class="icon-gap icon aufontAll h-icon-all-exclamation-circle"></i>
        {{ $t('cloudpivot.list.pc.importErrEdit.finishedTips') }}
      </span>
      <div class="flex">
                  ...
                  >
                    ...
                    <a-col :span="5">
                      <a-select
                        ...
                        class="data-filter"
                        :placeholder="
                          $t('cloudpivot.list.pc.importErrEdit.placeholder')
                        "
                      ...
                      <div class="batch-update-new-data">
                        ...
                        <div>
                          <a-tag v-if="row.value && row.value.length > 0">
                            <a-icon type="team" />
                            {{
                              $t('cloudpivot.list.pc.importErrEdit.seleted', {
                                num: row.value.length,
                              })
                            }}
                          </a-tag>
                        </div>
