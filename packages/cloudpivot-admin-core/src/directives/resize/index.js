...
const resizeTableBody = (el, newWidth) => {
  ...
  const targetColumn = tableContent.querySelectorAll(
...
export default {
  ...
  bind(el, binding) {
    ...
    if (binding.value && Object.keys(binding.value).length > 0) {
      ...
      if (
        ...
        typeof binding.value.onResize === 'function'
      ) {
  ...
  inserted(el) {
    const oDiv = el;
    oDiv._sizeStandard.minWidth = oDiv._sizeStandard.minWidth
      ? oDiv._sizeStandard.minWidth
      : oDiv.offsetWidth;
    const onMouseUp = (event) => {
    ...
    const onBodyMouseMove = (event) => {
      ...
      if (oDiv._dircetion.west) {
        if (
          event.pageX < oDiv._initData.x + 5 &&
          event.pageX > oDiv._initData.x - 5
        ) {
      ...
      if (oDiv._dircetion.north) {
        if (
          event.pageY < oDiv._initData.y + 5 &&
          event.pageY > oDiv._initData.y - 5
        ) {
      ...
      if (oDiv._dircetion.east) {
        if (
          event.pageX < oDiv._initData.offsetWidth + oDiv._initData.x + 10 &&
          event.pageX > oDiv._initData.offsetWidth + oDiv._initData.x - 5
        ) {
          oDiv.style.cursor = 'col-resize';
          oDiv.style.borderRight = '3px solid #2970FF';
          oDiv._allowToMove = true;
        } else {
          oDiv.style.cursor = 'default';
          oDiv.style.borderRight = '0';
        }
        if (oDiv._isMouseDown && oDiv._allowToMove) {
          event.preventDefault();
      ...
      if (oDiv._dircetion.south) {
        if (
          event.pageY < oDiv._initData.y + oDiv._initData.offsetHeight + 5 &&
          event.pageY > oDiv._initData.y + oDiv._initData.height - 5
        ) {
          oDiv.style.cursor = 'row-resize';
        } else {
          oDiv.style.cursor = 'default';
