...
<template>
  <div class="bind-server__table">
    ...
    >
      ...
      <span slot="parameterTitle" class="text-ellipsis resize">参数</span>
      <span slot="bizPropertyTypeTitle" class="text-ellipsis resize">参数类型</span>
      <span slot="codeTypeTitle" class="text-ellipsis resize">映射方法</span>
      <span slot="bizCodeTitle" class="text-ellipsis resize">映射对象</span>
      <!-- 表体 -->
      <span slot="parameter" slot-scope="text, record">{{
        record.serviceMethodParameterCode
      }}</span>
      <span slot="bizPropertyTypeName" slot-scope="text, record">{{
        record.bizPropertyTypeName || '--'
      }}</span>
      ...
      <div slot="bizCode" slot-scope="text, record">
        ...
        <a-select v-else v-model="record.bizCode" allowClear>
  ...
  dataItems(record: any, filterDefault: boolean = false) {
    ...
    if (filterDefault) {
      return this.dataItemList.filter(
        (item: any) => !selected.includes(item.code) && item.defaultProperty,
      );
    }
    return this.dataItemList.filter(
      (item: any) => !selected.includes(item.code) && !item.defaultProperty,
