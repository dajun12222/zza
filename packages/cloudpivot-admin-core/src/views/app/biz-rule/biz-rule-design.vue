    ...
    <div class="workflow-head" @click="closeContextmenu">
      ...
      <div class="workflow-head-r">
        <!-- <a-tooltip placement="top">
          <template slot="title"> 切换为编码方式 </template>
          <span
            class="icon-btn"
          ...
          >
            <i class="icon aufontAll h-icon-all-coding_o"></i>
          </span>
        </a-tooltip>
        <a-tooltip placement="top">
        ...
        <a-button class="btn" @click="changeEditMode">编码方式</a-button>
        <a-button class="btn" @click="checkRule">检测</a-button>
        <a-button
          class="btn"
...
export default class Workflow extends Vue {
  ...
  @BizRuleModule.Mutation('RESET_STATE') RESET_STATE: any;
  ...
  // 路由变化时检测做了修改的需要保存
  validRoute() {
    if (this.changeEditModeButtonClicked) {
      this.changeEditModeButtonClicked = false;
  ...
  changeEditModeButtonClicked: boolean = false;
  ...
  changeEditMode() {
    ...
    this.$confirm({
      title: '确定要切换为编码方式吗？',
      content:
        '可视化方式与编码方式不等价，切换方式将导致当前规则失效，请谨慎操作',
      class: 'change-edit-mode-confirm',
      width: '420px',
      icon: () =>
      ...
      onOk() {
        ...
        ruleParams.developmentMode = 1;
        ruleParams.code = vm.workflowData.code;
        ruleParams.schemaCode = params && params.bizSchemaCode;
        bizpropertyApi.changeBusinessMode(ruleParams).then((res: any) => {
          if (res.errcode === 0) {
            vm.changeEditModeButtonClicked = true;
            vm.$router
              .push({
                name: 'biz-rule-coding',
                params: { bizRuleCode: params.bizRuleCode },
                query: {
                  isEdit: `${query.isEdit}`,
                  code: query.code,
                  name: query.name,
