...
export default class BizRuleContent extends Vue implements H3Droppable {
  ...
  async mounted() {
    ...
    const bizRuleCode: string = this.$route.params.bizRuleCode as string;
    const id: string = (this.$route.query.id as string) || bizRuleCode;
    ...
    if (this.$route.query.isEdit === 'true') {
      if (bizRuleCode === 'dataOperation') {
        ...
        query.code =
          this.$route.query.code !== 'dataOperation'
            ? (this.$route.query.code as string)
            : '';
        query.name = (this.$route.query.name as string) || '';
        // 数据操作
        this.setDraft(dataOperation);
        this.setWorkflowData(Object.assign(new DataOperation(), query));
      } else if (bizRuleCode === 'regularOperation') {
        const query = { code: '', name: '' };
        query.code =
          this.$route.query.code !== 'regularOperation'
            ? (this.$route.query.code as string)
            : '';
        query.name = (this.$route.query.name as string) || '';
        // 定时作业
        this.setDraft(regularOperation);
        this.setWorkflowData(Object.assign(new RegularOperation(), query));
      } else {
      ...
      await this.getWorkflowDraft({ id: id });
      // 数据记载完成，初始属性栏
      // this.$emit('loadComplete');
      setTimeout(() => {
