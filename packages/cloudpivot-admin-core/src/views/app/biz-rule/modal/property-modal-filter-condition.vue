      ...
      <div class="centent">
        ...
        <div class="filters">
          ...
          <a-row style="margin-top: 10px">
            <a-col :span="7" class="fieldlab"> 目标数据字段 </a-col>
          ...
          >
            <a-col :span="7">
              <data-item-select
                ...
                :systemFilterCtl="{
                  code: filterWorkflowInstanceIdOrNot([
                    'workflowInstanceId',
                    'ownerDeptQueryCode',
                    'sortKey',
                    'modifier',
...
export default class PropertyFilterCondition extends Mixins(PropertyComponent) {
  ...
  schemaCode: string = this.$route.params.bizSchemaCode;
  ...
  async initModal() {
    ...
    await this.getTargetWorkflowList(schemaCode);
    await this.getCurWorkflowList(this.schemaCode);
  ...
  removeNotExistItem(type: string) {
    this.filtersList[type].map((item: any, index: number) => {
      ...
      if (
        targetdataItem &&
        targetdataItem.length === 0 &&
        ![
          SystemDataItemCode.ParentWorkflowId,
          SystemDataItemCode.ParentWorkflowSheetRowID,
        ].includes(item.targetSchemaDataItem)
      ) {
        ...
        this.filterCurrentItem(dataItem, item, type, index);
      }
    });
  ...
  async getDataItems(schemaCode: string) {
    ...
    if (res && res.errcode === 0) {
      ...
      //当前弹窗是查找数据范围
      if (this.showWorkflowDataItem) {
        //增加前端构造的流程数据项，父流程ID，父流程子表行ID
        this.addAdditionWorkflowItem();
      }
    } else {
  ...
  setRuleValueTypeList(targetItem, index, type, isInit) {
    ...
    if (
      [
        ...
        SystemDataItemCode.ParentWorkflowId,
        SystemDataItemCode.WorkflowInstanceId,
  ...
  setCurrentFieldList(targetDataItem, index, type) {
    ...
    //流程相关的一些数据项的映射关系
    if (this.showWorkflowDataItem) {
      this.workflowDataitemSetting(targetDataItem, index, type);
    }
      ...
      if (targetDataItem.code === SystemDataItemCode.Id) {
        this.filtersList[type][index].currentFieldList = this.filtersList[type][
          ...
          return (
            ...
            val.code !== SystemDataItemCode.WorkflowInstanceId
          );
        });
      }
    }
  ...
  get isUpdateNode() {
    return this.controller.value.dataTriggerType === 'UPDATE';
  }

  isCurWorkflow: boolean = false;

  isTargetWorkflow: boolean = false;

  async getTargetWorkflowList(schemaCode) {
    if (schemaCode) {
      const params = {
        schemaCode,
      };
      ...
      if (res.errcode === 0 && res.data && res.data.length) {
        this.isTargetWorkflow = true;
      } else {
        this.isTargetWorkflow = false;
      }
    }
  }

  async getCurWorkflowList(schemaCode) {
    if (schemaCode) {
      const params = {
        schemaCode,
      };
      ...
      const res = await WorkflowApi.getWorkflowList(params);
      if (res.errcode === 0 && res.data && res.data.length) {
        this.isCurWorkflow = true;
      } else {
        this.isCurWorkflow = false;
      }
    }
  }
  ...
  get showWorkflowDataItem() {
    return this.isUpdateNode && this.isCurWorkflow && this.isTargetWorkflow;
  }

  mockParentWorkflowIdItem: any = {
    code: SystemDataItemCode.ParentWorkflowId,
    createdBy: null,
  ...
  mockParentWorkflowSubRowIdItem: any = {
    code: SystemDataItemCode.ParentWorkflowSheetRowID,
    createdBy: null,
    defaultProperty: true,
    deleted: false,
    name: '父流程子表行业务对象ID',
  ...
  //为目标对象添加额外的流程属性：父流程ID
  addAdditionWorkflowItem() {
    this.targetFieldList.main.push({ ...this.mockParentWorkflowIdItem });
  }

  workflowDataitemSetting(targetDataItem, index, type) {
    if (targetDataItem.propertyType === DataItemType.ShortText) {
      //过滤系统字段
      if (
        [
          SystemDataItemCode.ParentWorkflowId,
          SystemDataItemCode.WorkflowInstanceId,
        ].includes(targetDataItem.code)
      ) {
        this.filtersList[type][index].currentFieldList = this.filtersList[type][
          index
        ].currentFieldList.filter((item) => {
          return (
            !item.defaultProperty ||
            item.code === SystemDataItemCode.WorkflowInstanceId
          );
        });
      }

      //“父流程子表行ID”可与单行文本、子表业务对象ID条件对比
      if (
        type === 'child' &&
        (!targetDataItem.defaultProperty ||
          targetDataItem.code === SystemDataItemCode.Id)
      ) {
        //加入父流程子表行ID
        this.filtersList[type][index].currentFieldList.push({
          ...this.mockParentWorkflowSubRowIdItem,
        });
      }

      //“流程实例ID”可与单行文本、“父流程ID”对比
      if (
        targetDataItem.defaultProperty &&
        !(targetDataItem.code === SystemDataItemCode.ParentWorkflowId)
      ) {
        //加入流程实例ID
        this.filtersList[type][index].currentFieldList = this.filtersList[type][
          index
        ].currentFieldList.filter((item) => {
          return item.code !== SystemDataItemCode.WorkflowInstanceId;
        });
      }

      //“父流程ID”可与单行文本、“流程实例ID”对比
      if (
        !targetDataItem.defaultProperty ||
        targetDataItem.code === SystemDataItemCode.WorkflowInstanceId
      ) {
        //加入父流程ID
        this.filtersList[type][index].currentFieldList.push({
          ...this.mockParentWorkflowIdItem,
        });
      }
    }
  }

  //根据是否需要展示流程数据项，判断是否删除流程实例ID系统字段
  filterWorkflowInstanceIdOrNot(dataItems: any[]) {
    return dataItems.filter((item) => {
      return (
        !this.showWorkflowDataItem &&
        item === SystemDataItemCode.WorkflowInstanceId
      );
    });
  }

  filterCurrentItem(dataItem, item, type, index) {
    if (
      dataItem &&
      dataItem.length === 0 &&
      ![
        SystemDataItemCode.ParentWorkflowId,
        SystemDataItemCode.ParentWorkflowSheetRowID,
      ].includes(item.currentSchemaDataItem)
    ) {
