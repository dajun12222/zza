...
<template>
  <div class="package-setting">
    <div v-if="!isTabPane" class="package-setting__header">
      <h3>{{ $t('languages.appSettings.packageSetting') }}</h3>
      <a-button type="primary" icon="save" @click="bing">
        {{ $t('languages.appSettings.bind') }}
      </a-button>
    </div>
    <div class="info">
      <a-alert
    ...
    <div v-if="isTabPane" class="package-setting__footer">
      <a-button type="primary" @click="bing">
        {{ $t('languages.appSettings.bind') }}
      </a-button>
    </div>
  </div>
...
<script lang="ts">
import { Alert, Input, Button } from '@h3/antd-vue';
...
export default class PackageSetting extends Vue {
  ...
  bing() {
    if (Object.values(this.form).some((val: string) => !val)) {
      this.$message.warning(
        this.$t('languages.appSettings.PlzFillInInfo') as string,
      );
      return;
    }
    // submit
    this.updateAppPackage(this.form).then((res: Common.Data) => {
      if (res.errcode) {
        ...
        this.$message.success(
          this.$t('languages.appSettings.BindSuccessfully') as string,
        );
      }
    });
  }
  ...
  mounted() {
    const { appKey, appSecret, agentId } = this.appInfo;
    this.form = {
      appKey,
      appSecret,
      agentId,
