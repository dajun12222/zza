  ...
  <div class="biz-rule-info-detail">
    ...
    <div class="biz-rule-info-detail-msg">
      <a-row>
        ...
        <a-col :span="8">
          {{ selectData.functionName }}
        </a-col>
      </a-row>
      <a-row>
        <a-col class="info-label" :span="4"> 业务规则名称： </a-col>
        <a-col :span="8">
          {{ detailData.businessRuleLogHeaderModel.businessRuleName }}
        </a-col>
        <a-col class="info-label" :span="4"> 业务规则编码： </a-col>
        <a-col :span="8">
          {{ detailData.businessRuleLogHeaderModel.businessRuleCode }}
        </a-col>
      </a-row>
      <a-row>
        ...
        <a-col
          :span="8"
          :class="[
            detailData.businessRuleLogHeaderModel.success ? 'success' : 'fail',
          ]"
        >
          {{ detailData.businessRuleLogHeaderModel.success ? '成功' : '失败' }}
        </a-col>
        <a-col class="info-label" :span="4"> 异常节点： </a-col>
        <a-col :span="8">
          {{
            !detailData.businessRuleLogContentModel.exceptionNodeName
              ? '-'
              : detailData.businessRuleLogContentModel.exceptionNodeName
          }}
        </a-col>
      ...
      <a-row>
        ...
        <a-col :span="8">
          {{ detailData.businessRuleLogHeaderModel.endTime }}
        </a-col>
        <a-col class="info-label" :span="4"> 执行开始时间： </a-col>
        <a-col :span="8">
          {{ detailData.businessRuleLogHeaderModel.startTime }}
        </a-col>
      </a-row>
      <a-row>
        ...
        <a-col :span="8">
          {{
            detailData.businessRuleLogHeaderModel.repair
              ? '修复动作'
              : '系统调用'
          }}
        </a-col>
    ...
    <div class="biz-rule-info-detail-implementstate">
      <a-row>
        ...
        <a-col :span="20">
          {{
            !detailData.businessRuleLogContentModel.exceptionContent
              ? '无'
              : detailData.businessRuleLogContentModel.exceptionContent
          }}
        </a-col>
    ...
    <div class="biz-rule-info-detail__footer">
      ...
      <a-button
        v-if="detailData.businessRuleLogHeaderModel.showRetryBtn"
        type="primary"
...
import { Row, Col, Divider, Button } from '@h3/antd-vue';
  ...
  rollBack() {
    if (this.detailData && this.detailData.businessRuleLogHeaderModel) {
      if (!this.isRollBack) {
        ...
        const flowInstanceId: string =
          this.detailData.businessRuleLogHeaderModel.flowInstanceId || '';
        flowInstanceId &&
          systemManageApi.retryBusinessRule(flowInstanceId).then(
            (res) => {
