  ...
  <div class="config-rule">
    ...
    <div class="config-rule__item">
      ...
      >
        ...
        <p class="item__info">
          {{ content.content }}
          <a v-if="content.url" target="_blank" :href="content.url">
            {{ content.url }}</a>
        </p>
        <div class="item__img">
...
export default class configRule extends Vue {
  dingTalkCheck = [
    ...
    },
    {
      title: '应用首页链接：',
      content:
        '云枢系统移动端访问地址，格式为：http://SERVER/mobile?corpId=$CORPID$&agentId=xxx，其中，XXX为当前应用的AgentId，创建后获取 服务器出口IP：云枢系统的IP地址，每个IP只能在一个企业中使用 PC端首页地址：云枢系统访问PC端的地址，格式：http://SERVER?corpId=$CORPID$&agentId=xxx 管理后台地址：云枢系统访问PC端的地址，格式：http://SERVER/admin',
      url: '',
    ...
    {
      title:
        '生成应用，获取Appkey、APPSecret和AgentId，将AgentId填写到地址中，更新应用，并将信息填写到云枢系统“钉钉集成”中。',
      content: '',
      url: '',
  ...
  portalSetting = [
    {
      title:
        'CorpID、SSOsecret：钉钉企业的唯一标识，获取地址：工作台-自建应用-开发信息-开发账号管理-企业自用账号信息。',
      content: '',
      url: '',
      img: [require('./images/04.png')],
    },
    {
      title:
        '扫码登录app ID、扫码登录appSecret、移动接入应用的标识，获取地址：钉钉后台，工作台-自建应用--应用设置-移动介入应用-扫码登录使用的app ID、appSecret获取',
      content: '',
      url: '',
