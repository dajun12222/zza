...
<template>
  <div class="content-box">
    <div v-if="rendererOptions.layout.length > 0" class="clearfix">
      <smart-form :preview="rendererOptions" />
      <div class="btn-group">
        <a-button class="btn">
          {{ $t('languages.Apps.Reset') }}
        </a-button>
...
import { buildControlOf } from '../form-design/typings/control-factory';
...
export default class QueryCrieria extends Vue {
  ...
  setControlData(code: string, index: number) {
    if (this.payloadOptions[index].data) {
      ...
      if (propertyType === 5) {
        controlsDefault.defaultValue =
          this.staffSelectorDefaultMap[defaultValue];
      }
