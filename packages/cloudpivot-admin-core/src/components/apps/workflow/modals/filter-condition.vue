                ...
                <a-col class="filterw100" :span="8">
                  ...
                  >
                    <template v-for="(d, i) in row.conditionTypeList">
                      ...
                      <!-- <a-select-option
                        v-if="ifDisplayOption(row.currentDataItem, d.code)"
                        :key="i"
                        :value="d.code"
                      > -->
                  ...
                  >
                    <a-tooltip :visible="!!row.errorMsg">
                      ...
                      <div :class="!!row.errorMsg && 'has-error'" class="value">
                        ...
                        <StaffSelector
                          v-else-if="
                            [5, 50, 51, 60, 61].includes(row.propertyType)
                          "
                          v-model="row.value"
                          :onlyForm="true"
...
<script lang="ts">
import { BizRuleDataCondition } from 'cloudpivot-admin-core/src/views/app/biz-rule/typings/filter-condition';
import {
  Modal,
  Row,
  Col,
...
export default class FilterCondition extends Vue {
  ...
  getStaffSelectorOpts(type) {
    return {
      selectOrg: [5, 60, 61].includes(type),
      selectUser: [50, 51].includes(type),
      mulpitle: [5, 51, 61].includes(type),
      showModel: true,
      showSelect: true,
      placeholder: '请选择',
    };
  ...
  watchFiltersList(val) {
    this.updateConditionDataItemDisable();
  }

  updateConditionDataItemDisable() {
  ...
  addRows(index: number): void {
    ...
    if (Array.isArray(options) && options.length) {
      const newRow: ITableCondition = {
        ...
        propertyType: '',
        errorMsg: '',
        targetFieldList: JSON.parse(JSON.stringify(this.targetFieldList)),
      };
  ...
  //   const dataItem = this.targetFieldList.find((item: any) => {
  //     return item.code === propertyCode;
  //   });

  //   if (dataItem.propertyType === DataItemType.DeptSingle) {
  //     return (
  //       dataItem.isSystem ||
  //       !['OF', 'NOF'].includes(value) ||
  //       (['OF', 'NOF'].includes(value) && dataItem.backupQueryCode)
  //     );
  //   } else {
  //     return true;
  //   }
