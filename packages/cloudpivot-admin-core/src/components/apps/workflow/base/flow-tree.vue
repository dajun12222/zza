...
import WorkflowCenter from 'cloudpivot-admin-core/src/apis/workflow';
...
export default class FlowTree extends Vue {
  ...
  private async getFolderOrBizModelsByCode(appCode: string) {
    ...
    const result: any = await await AppCenter.getAppItem(params);
  ...
  async getWorkFlowTreeData(schemaCode: string) {
    ...
    const workflowList: Array<any> = result.data;
    // .filter(
   ...
   * 选中的时候
   */
  async onSelect(value: any, vNode: any) {
  ...
  reduceTreeCondition(item) {
    const visibleType = ['Group', null].includes(item.type);
