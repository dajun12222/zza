...
import { formDirective } from 'cloudpivot-admin-core/src/directives/form-validate';
import FormValidateHandleMixin from 'cloudpivot-admin-core/src/minxins/form-validate-handle';
import { Collapse, Tabs } from '@h3/antd-vue';
import {
  commonBaseAttributeListMap,
  controlAttributeListMap,
} from 'cloudpivot-form/form/src/schema/control-attribute-transfer';
import {
  CompareVersion,
...
export default class FormAttribute extends mixins(FormValidateHandleMixin) {
  ...
  emitControlDelete(controlKey: string, replace = false) {
    this.$emit('control-delete', controlKey, replace);
  }

  selectControl(selectControl) {
    this.$emit('control-select', selectControl);
  }
  ...
  setAttribute(allControls: any) {
    ...
    if (componentAsset) {
      ...
      for (const attributeKey of Object.keys(groups)) {
        ...
        for (const key of attribute.keys) {
          ...
          if (properties && properties[key]) {
            ...
            if (dataItem) {
              ...
              // 部门单选控件新增基础属性————是否作为查询条件，数据项入库后需设置为只读
              // if (key === 'backupQueryCode') {
              //   if (!controlAttribute.options) {
              //     controlAttribute.options = {};
              //   }
              //   controlAttribute.options.disabled = dataItem.id;
              // }
            }
            // 关联查询用于区分是关联单选还是关联多选
            if (key === 'relativeType') {
            ...
            if (properties && properties[key]) {
              ...
              if (mapItem) {
                if (mapItem.isRuleAttribute === 0) {
                  ...
                  if (
                    ['requiredFormula'].includes(controlAttribute.field) &&
                    this.formData.modelType === 'TREE' &&
                    controlAttribute.dataItem.type === 9 &&
                    controlAttribute.dataItem.code === 'parentId'
                  ) {
                    // 树形模型的上级关联 不显示是否必填
                    controlAttribute.display = false;
                  }
        ...
        setTimeout(() => {
          // 树形模型上级关联 控件类型、绑定数据字段、选择业务模型、数据项类型、显示样式不能修改
          attributeColumns.forEach((element) => {
            if (
              this.formData.modelType === 'TREE' &&
              element.dataItem.type === 9 &&
              element.dataItem.code === 'parentId'
            ) {
              if (
                [
                  'widgetType',
                  ...
                  'schemaCode',
                  'showStyle',
                ].includes(element.field)
              ) {
                // 禁止修改
                element.options.disabled = true;
              } else if (['showField', 'isAuthorize'].includes(element.field)) {
                // 不显示
