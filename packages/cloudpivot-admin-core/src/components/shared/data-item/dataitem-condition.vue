...
export default class DataitemCondition extends Vue {
  ...
  dataOptions: {
    ...
    type: DataItemType;
    disabled: boolean;
    // backupQueryCode: boolean; // 是否作为查询条件
  }[] = [];
  ...
  showSelfInput(cond: DataitemConditionItem) {
    ...
    const res =
      ...
      !item.isSystem &&
      ![DateItemOperatorType.Of, DateItemOperatorType.NotOf].includes(
        cond.operatorType,
      );
    // 设置默认值
    // if (res) {
  ...
  getOperators(type: DataItemType, code: string) {
    ...
    if (
      [DataItemType.DeptSingle].includes(type) &&
      !isSystem
      // !isSystem &&
      // item.backupQueryCode
    ) {
      const Of = [
        {
          label: '属于',
      ...
      dataItemOperators = [...bussinessStaffDataItemOperators, ...Of];
    }
        ...
        this.dataOptions = items
          ...
          .map((item) => {
            ...
            return {
              ...
              disabled,
              // backupQueryCode: item.backupQueryCode,
            };
          });
        // this.add();
