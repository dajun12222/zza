        ...
        <basic-box @destroy="destroy">
          ...
          <div slot="table" class="table-box">
            ...
            <a-row class="row-flex">
              ...
              <a-col
                :span="20"
                class="content text-ellipsis clamp3"
                :title="data.detail"
              >
                {{ data.detail }}
              </a-col>
          ...
          <div slot="table" class="table-box">
            ...
            >
              ...
              <template slot="participantName" slot-scope="text, record">
                <span class="fake-btn text-ellipsis">{{
        ...
        <basic-box @destroy="destroy">
          ...
          <div slot="table" class="table-box">
            ...
            >
              ...
              <template slot="participantName" slot-scope="text, record">
                <span
                  :title="record.participantName"
                  class="text-ellipsis fake-btn"
                  >{{ record.participantName }}</span>
              </template>
              <!-- 操作 -->
              <span slot="operateTitle" class="resize">操作</span>
              <template slot="operate" slot-scope="text, record, index">
                <span
                  v-if="record.circulate"
                  class="btn"
                  @click="btnOperate('circulate', record, index)"
                  >已阅</span>
                <span
                  v-if="!record.circulate && record.actionPermissionVO.submit"
                  class="btn"
                  @click="btnOperate('submit', record, index)"
                  >提交</span>
                <span
                  v-if="!record.circulate && record.actionPermissionVO.agree"
                  class="btn"
                  @click="btnOperate('approve', record, index)"
                  >同意</span>
                <span
                  v-if="!record.circulate && record.actionPermissionVO.disAgree"
                  class="btn"
                  @click="btnOperate('disagree', record, index)"
                  >不同意</span>
                <span
                  v-if="
                    !record.circulate && record.actionPermissionVO.showReject
                  "
                  class="btn"
                  @click="btnOperate('reject', record, index)"
                  >驳回</span>
                <span
                  v-if="!record.circulate"
                  class="btn"
                  @click="btnOperate('more', record, index)"
                  >更多</span>
              </template>
            </a-table>
          </div>
...
<script lang="ts">
import { Row, Col, Table } from '@h3/antd-vue';
...
export default class MockChartCard extends Vue {
  ...
  btnOperate(type, record, index) {
    ...
    this.loading = true;
    this.fn({ type, record }, (err, { isFlowNext, fullScreen }) => {
      //操作完后,需要根据baseInfo当前流程节点是否已处理完,处理完后弹框消息
      if (err) {
        this.destroy();
      } else if (!err && (isFlowNext || fullScreen || len === 1)) {
        // 如果是放大或者已经流转到下一个节点
        this.destroy();
      } else if (!err && len > 1) {
        //如果有多条数据,没有流转到下一个节点,需要继续操作
  ...
  get positionStyle() {
    ...
    let pageX: number = 0;
    let pageY: number = 0;
    const boxWidth: number = this.data.length > 1 ? 278 : 154;
    ...
    return `top:${pageY}px;left:${pageX}px;width:${
      widthMap[this.cardType || 'initial']
    }px`;
  }
