...
import { Activity } from 'cloudpivot-admin-core/src/typings/workflow';
...
export const calculateDockActivity = (
  ...
  activities.some((curActivity: Activity) => {
    if (
      ...
      //自动吸附逻辑
      const sorbPoint = sorbToRecentDot(curActivity, dockPosition);
      dockPosition.x = sorbPoint.x;
      dockPosition.y = sorbPoint.y;
  ...
  if (!dockPosition.Activity) {
    activities.some((target: Activity) => {
      if (
        ...
        //自动吸附逻辑
        const sorbPoint = sorbToRecentDot(target, dockPosition);
        dockPosition.x = sorbPoint.x;
        dockPosition.y = sorbPoint.y;
...
const autoFitScroll = (evt: any) => {
  //如果鼠标拖拽悬浮到了节点上方，不做处理直接返回
  if (
    !evt.srcElement ||
    (evt.srcElement.classList?.length > 0 &&
      !Array.from(evt.srcElement.classList).includes('lines-instance'))
  ) {
    return;
  }

  const designerWrapDom: any = document.querySelector('#designer-wrap');
  // const reduceWidth = designerWrapDom.clientWidth / 16;
  // const reduceHeight = designerWrapDom.clientWidth / 16;
  const triggerArea = 100;
  // console.log(
  //   'scrollW:',
  //   designerWrapDom.scrollWidth,
  //   'clientW:',
  //   designerWrapDom.clientWidth,
  //   '偏移：',
  //   evt.offsetX,
  //   'scrollLeft:',
  //   designerWrapDom.scrollLeft,
  // );

  // 根据流程连接线拖拽位置判断是否需要滚动画布

  //左右方向
  if (
    designerWrapDom.clientWidth - evt.offsetX + designerWrapDom.scrollLeft <
    triggerArea
  ) {
    const reduceWidth =
      triggerArea -
    ...
    if (
      ...
      designerWrapDom.scrollLeft += reduceWidth;
    } else {
      designerWrapDom.scrollLeft =
        designerWrapDom.scrollWidth - designerWrapDom.clientWidth;
    }
  }

  if (evt.offsetX - designerWrapDom.scrollLeft < triggerArea) {
    const reduceWidth =
      triggerArea - (evt.offsetX - designerWrapDom.scrollLeft);
    if (designerWrapDom.scrollLeft > reduceWidth) {
      designerWrapDom.scrollLeft -= reduceWidth;
    } else {
      designerWrapDom.scrollLeft = 0;
    }
  }

  //上下方向
  if (
    designerWrapDom.clientHeight - evt.offsetY + designerWrapDom.scrollTop <
    triggerArea
  ) {
    const reduceHeight =
      triggerArea -
      (designerWrapDom.clientHeight - evt.offsetY + designerWrapDom.scrollTop);
    if (
      designerWrapDom.scrollHeight -
        designerWrapDom.clientHeight -
        designerWrapDom.scrollTop >
      reduceHeight
    ) {
      designerWrapDom.scrollTop += reduceHeight;
    } else {
      designerWrapDom.scrollTop =
        designerWrapDom.scrollHeight - designerWrapDom.clientHeight;
    }
  }

  if (evt.offsetY - designerWrapDom.scrollTop < triggerArea) {
    const reduceHeight =
      triggerArea - (evt.offsetY - designerWrapDom.scrollTop);
    if (designerWrapDom.scrollTop > reduceHeight) {
      designerWrapDom.scrollTop -= reduceHeight;
    } else {
      designerWrapDom.scrollTop = 0;
    }
  }
};

/**
 * @description 拖拽时，当与节点某个中点的位置小于一定值时，自动吸附到该中点
 * @param node 目标节点
 * @param movePoint 拖拽事件的坐标，{x: ?, y: ?}
 * @returns 拖拽事件吸附后的坐标，{x: ?, y: ?}
 */
export const sorbToRecentDot = (node, movePoint) => {
  const originOffsetX = 4;
  const originOffsetY = 3;
  const result = {
    x: movePoint.x,
    y: movePoint.y,
  };
  if (
    Math.abs(movePoint.x + originOffsetX - (node.left + node.right) / 2) <
    Line.settings.DockDisntaceToEdgeCenter
  ) {
    result.x = (node.left + node.right) / 2;
  }
  if (
    Math.abs(movePoint.y + originOffsetY - (node.top + node.bottom) / 2) <
    Line.settings.DockDisntaceToEdgeCenter
  ) {
    result.y = (node.top + node.bottom) / 2;
  }
  return result;
};
