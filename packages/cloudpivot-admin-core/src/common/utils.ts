...
function clone(x) {
  ...
  while (stack.length) {
    ...
    const uniqueData: any = uniqueList.find((item) => item.source === data);
    if (uniqueData) {
    ...
    for (const k in data) {
      if (data.hasOwnProperty(k)) {
        if (data[k] === null || typeof data[k] !== 'object') {
          // 基础类型克隆
          const descriptor: any = Object.getOwnPropertyDescriptor(data, k);
