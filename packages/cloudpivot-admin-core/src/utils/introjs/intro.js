...
export default (function introJs() {
  ...
  function _introForElement(targetElm, group) {
    ...
    if (this._options.steps) {
      ...
      _forEach(
        this._options.steps,
        function (step) {
          const currentItem = _cloneObject(step);
  ...
  function _placeTooltip(
    ...
    tooltipLayer.className = ('introjs-tooltip ' + tooltipCssClass).trim();
    ...
    const tooltipOffset = _getOffset(tooltipLayer);
  ...
  function _showElement(targetElement) {
    ...
    if (oldHelperLayer !== null) {
      ...
      const h3StepNumberLayer = oldReferenceLayer.querySelector(
          '.introjs-currentStepNumber',
        ),
      ...
      if (this._options.showStepNumbers === true) {
        const stepNumbersLayer = document.createElement('span');
        const stepsCount = this._options.steps.length;
      ...
      nextTooltipButton.onclick = function () {
        //  // mark scrollIntoView
        // let id = self._options.steps[self._currentStep + 1].element;
        // if (id) {
        //   let targetEle = document.querySelector(id);
    ...
    const tooltipButtons = targetElement.tooltipButtons;
  ...
  function _setShowElement(targetElement) {
    ...
    const currentElementPosition = _getPropValue(
      targetElement.element,
      'position',
    );
    if (
  ...
  function _removeClass(element, classNameRegex) {
    if (element instanceof SVGElement) {
      ...
      element.setAttribute('class', pre.replace(classNameRegex, '').trim());
    } else {
  ...
  function _populateHints(targetElm) {
    ...
    if (this._options.hints) {
      _forEach(
        this._options.hints,
        function (hint) {
          const currentItem = _cloneObject(hint);
  ...
  function _hideHint(stepId) {
    const hint = _hintQuerySelectorAll(
   ...
   */
  function _hideHints() {
    const hints = _hintQuerySelectorAll('.introjs-hint');
