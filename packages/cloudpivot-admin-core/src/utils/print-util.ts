...
export function mouseBounds(
  ...
  const tempObj: any = compRect.toJSON();
  ...
  if (item.eleType.includes('column')) {
    ...
    fourBorders =
      2 *
      (parseInt(item.rightValue.borderWidthValue, 10) +
        parseInt(item.leftKey.borderWidthValue, 10));
    // @ts-ignore
    twoBorders =
      parseInt(item.rightValue.borderWidthValue, 10) >
      parseInt(item.leftKey.borderWidthValue, 10)
    ...
    item.eleType.includes('text') ||
    item.eleType.includes('topDrawerRect')
  ) {
    // @ts-ignore
  ...
  if (item.eleType.includes('column')) {
    ...
    if (str.includes('left')) {
      tempObj.right += item.rightValue.widthValue + fourBorders;
    }

    // @ts-ignore
    if (str.includes('right')) {
      tempObj.left -= item.leftKey.widthValue + fourBorders;
    }
...
export function forRightPosition(x: number, y: number, item: object): object {
  ...
  // @ts-ignore
  if (item.owner.includes('left')) {
    // 左侧拖过来的字段，初始化
    // @ts-ignore
    if (item.kind.includes('leftColumn')) {
      // 左侧的系统、业务字段
      // @ts-ignore
    ...
    // @ts-ignore
    if (item.kind.includes('leftQrcodePic')) {
      // 左侧的二维码
      // @ts-ignore
    ...
    // @ts-ignore
    if (item.kind.includes('leftBarcodePic')) {
      // 左侧的条形码
      // @ts-ignore
...
export function onChange(inputValue: number, str: string, that: any) {
  // 先具体，后续抽象
  if (inputValue < 0) {
    that.$message.warning('请输入大于0的数值！');
    return;
  }
  ...
  if (kind.includes('column')) {
    if (that.leftOrRight.includes('left')) {
      oneColumnWidth =
        that.itemAttrs.leftKey.widthValue +
        2 * parseInt(that.itemAttrs.leftKey.borderWidthValue, 10);
    }
    if (that.leftOrRight.includes('right')) {
      oneColumnWidth =
        that.itemAttrs.rightValue.widthValue +
        2 * parseInt(that.itemAttrs.rightValue.borderWidthValue, 10);
    }
    // 字段column的总宽
    columnAllWidth =
      that.itemAttrs.leftKey.widthValue +
      that.itemAttrs.rightValue.widthValue +
      2 *
        (parseInt(that.itemAttrs.leftKey.borderWidthValue, 10) +
          parseInt(that.itemAttrs.rightValue.borderWidthValue, 10)); // column的总宽
    // 字段column的总高
    columnHeight =
      2 *
        parseInt(
          that.itemAttrs.leftKey.borderWidthValue >
            that.itemAttrs.rightValue.borderWidthValue
            ? that.itemAttrs.leftKey.borderWidthValue
            : that.itemAttrs.rightValue.borderWidthValue,
          10,
        ) +
      that.itemAttrs.heightValue; // column的总高
  }
  // @ts-ignore
  const textAllWidth: number =
    2 * parseInt(that.itemAttrs.borderWidthValue, 10) +
    that.itemAttrs.widthValue; // text的总宽
  // @ts-ignore
  const itemAllHeight: number =
    2 * parseInt(that.itemAttrs.borderWidthValue, 10) +
    that.itemAttrs.heightValue; // text、rect的总高
  // @ts-ignore
  ...
  if (str.includes('x')) {
    ...
    // @ts-ignore
    if (
      (kind.includes('text') || kind.includes('topDrawerRect')) &&
      inputValue > 492 - textAllWidth
    ) {
      // @ts-ignore
    ...
    // @ts-ignore
    if (
      (kind.includes('topDrawerLine') || kind.includes('Pic')) &&
      inputValue > 492 - lineAndAllPicWidth
    ) {
      // @ts-ignore
  ...
  if (str.includes('y')) {
    ...
    if (
      (kind.includes('topDrawerLine') || kind.includes('Pic')) &&
      inputValue > 695 - lineAndAllPicHeight
    ) {
      // @ts-ignore
  ...
  if (str.includes('w')) {
    ...
    // @ts-ignore
    if (
      (kind.includes('text') || kind.includes('topDrawer')) &&
      inputValue < 85
    ) {
      // @ts-ignore
    ...
    if (kind.includes('column')) {
      if (that.leftOrRight.includes('left')) {
        ...
        if (inputValue > allowWidth - that.itemAttrs.rightValue.widthValue) {
          that.itemAttrs.leftKey.widthValue =
            allowWidth - that.itemAttrs.rightValue.widthValue;
          that.$message.warning(warningTxt.overWh);
        }
      }
      if (that.leftOrRight.includes('right')) {
        ...
        if (inputValue > allowWidth - that.itemAttrs.leftKey.widthValue) {
          that.itemAttrs.rightValue.widthValue =
            allowWidth - that.itemAttrs.leftKey.widthValue;
          that.$message.warning(warningTxt.overWh);
        }
      }
    }
