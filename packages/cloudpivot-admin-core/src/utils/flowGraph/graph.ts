...
export default class NodeGraph {
  ...
  constructor(links: Array<NodeGraphType.Link>) {
    ...
    const allNodes = outNodes;
    inNodes.forEach((node: NodeGraphType.Node) => {
      if (
        allNodes.some((item: NodeGraphType.Node) => item.code === node.code)
      ) {
  ...
  getRoute(node: NodeGraphType.Node) {
    ...
    const currentRoute: NodeGraphType.Route = {
      originNode: node,
      crossNodes: [],
      arrowNode: null,
    };
    ...
    const pickNextNode = (targetNode: NodeGraphType.Node) => {
      const linkItem = this.runningLinks.find(
        (link: NodeGraphType.Link) => link.start.code === targetNode.code,
      );
       ...
       * 移除访问过的流
       */
      this.runningLinks = this.runningLinks.filter(
        (link: NodeGraphType.Link) =>
          link.start.code !== targetNode.code ||
          link.end.code !== nextTarget.code,
      );
    ...
    return new Promise((resolve) => {
      ...
      this.nodes.forEach((node: NodeGraphType.Node) => {
        ...
        if (route && route.crossNodes.length) {
          const path = route.crossNodes
            .map((cn: NodeGraphType.Node) => cn.name)
            .join(' > ');
          vehicle.push({
            ...route,
            path,
