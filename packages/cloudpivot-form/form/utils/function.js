export default [
  {
    ...
    example:
      '计算出学生成绩中平均分。【成绩明细】里面有3个科目，每个科目分数不同，【平均分】显示这三个科目【成绩】的平均分值',
    type: 'number',
  },
  {
    code: 'STRING()',
    ...
    usage:
      'STRING({字段1},{字段2},…)，支持短文本、单选文本、多选文本、日期、整数、含小数数值、人员单选、人员多选、部门单选、部门多选、人员部门混选类型的字段',
  ...
  },
  {
    ...
    desc: '(【参数】,s,e)，文本截取函数',
    usage:
      'SUBSTRING(数据项,开始位置,结束位置)，当数据项内容的长度小于开始位置或小于结束位置时，不执行该计算函数',
    example:
      'SUBSTRING(数据项,2,7)，数据项的内容为“奥哲云枢低代码平台”，会返回“云枢低代码”，数据项的内容为“奥哲”或“奥哲云枢”，不返回任何值；数据项只支持短文本类型的数据项。',
    type: 'text',
  },
  {
    code: 'REPLACE()',
    ...
    desc: 'REPLACE',
    usage:
      'REPLACE(old_text,start_num,num_chars,new_text)，old_text为某文本字符串，start_num为要替换的起始位置编号，num_chars为要替换的字符个数，new_text为替换后的字符串',
  ...
  },
  {
    ...
    usage:
      'UPPERMONEY(数值)，在和金额相关的系统中，为了符合财务的标准或防涂改等，采用此函数即可将数值型转为中文大写金额',
  ...
  },
  {
    ...
    usage:
      'IF(数据项1>n or 数据项2=数据项3,"显示值1","显示值2")，数据项是"数值,文本，单选，复选，下拉单选，下拉复选，日期，"',
    example:
      'IF(成绩>60,"及格","不及格")录入【成绩】，通过IF函数的比对自动填入【及格】【不及格】',
    remark:
      '显示值为常量需要英文单引号""，比如"及格"；如果显示值为数据项，不需要添加英文双引号。比如某数据项。',
    type: 'text',
  },
  {
    code: 'SUM()',
    ...
    example:
      '计算学生成绩总分。【成绩明细】里面有3个科目，【总分】显示这三个科目的成绩总和',
    type: 'number',
  },
  {
    code: 'MAX()',
    ...
    example:
      '计算出学生成绩中最高分。【成绩明细】里面有3个科目，每个科目分数不同，【最高分】显示这三个科目【成绩】最高的分值',
    type: 'number',
  },
  {
    code: 'MIN()',
    ...
    example:
      '计算出学生成绩中最低分。【成绩明细】里面有3个科目，每个科目分数不同，【最低分】显示这三个科目【成绩】最低的分值',
    type: 'number',
  },
  {
    code: 'COUNT()',
    ...
    example:
      '计算【学生成绩】中的科目数，【科目数】显示【成绩明细】中科目的数量',
    type: 'number',
  },
  {
    code: 'COUNTIF()',
    ...
    example:
      'COUNTIF(子表.性别,"男") 返回性别男的人数， COUNTIF(子表.年龄, ">18") 返回年龄大于18的人数， COUNTIF(子表.年龄, 18)返回年龄等于18的人数',
    type: 'number',
  },
  {
    code: 'CONTAINS()',
    ...
    example:
      '当用章类型不选择时，默认隐藏公用章、合同章控件，选择“公用章”时，显示公用章控件，选择“合同章”时，则显示合同章控件',
    type: 'text',
  },
  {
    code: 'GETADDRESS()',
  ...
  },
  {
    ...
    example:
      '当数值相除后小数很多的情况，可以利用ROUND进行四舍五入到指定位数。【均价】四舍五入后保留位小数',
    type: 'number',
  },
  {
    code: 'INTUP()',
    ...
    example:
      '在计算采购桌子数量时，按人头一除，需要在小数点前面向上取整数来满足采购场景',
    type: 'number',
  },
  {
    code: 'INTDOWN()',
    ...
    example:
      '用于计算和统计时，比如预留座位，除以人头，32.1个座位，那么只能接纳32个人',
    type: 'number',
  },
  {
    code: 'DATEDIF()',
    ...
    usage:
      'DATEDIF(【结束时间】,【开始时间】,【单位】)，单位可以是”y”、”M”、”d”、”h”、”m“、”s”',
    example:
      'DATEDIF(结束时间,开始时间,”h”)，如果下单时间是9:00，付款时间是当天10:30，计算得到的小时差为1.5小时。',
    type: 'date',
  },
  {
    code: 'YEAR()',
    ...
    usage:
      'YEAR(时间戳)，时间戳可以是日期类型的数据项，也可以之间输入时间格式的文本',
  ...
  },
  {
    ...
    usage:
      'MONTH(时间戳)，时间戳可以是日期类型的数据项，也可以之间输入时间格式的文本',
  ...
  },
  {
    ...
    usage:
      'DAY(时间戳)，时间戳可以是日期类型的数据项，也可以之间输入时间格式的文本',
  ...
  },
  {
    ...
    usage:
      'QUARTER(时间戳)，时间戳可以是日期类型的数据项，也可以之间输入时间格式的文本',
  ...
  },
  {
    ...
    usage:
      'HOUR(时间戳)，时间戳可以是日期类型的数据项，也可以之间输入时间格式的文本',
  ...
  },
  {
    ...
    usage:
      'MINUTE(时间戳)，时间戳可以是日期类型的数据项，也可以之间输入时间格式的文本',
  ...
  },
  {
    ...
    usage:
      'SECOND(时间戳)，时间戳可以是日期类型的数据项，也可以之间输入时间格式的文本',
  ...
  },
  {
    ...
    example:
      'ADDDAY(项目开始时间,项目天数)，录入【项目开始时间】、【项目预计天数】，通过ADDDAY函数自动填入【项目预计完成时间】',
    type: 'date',
  },
  {
    code: 'ADDMONTH()',
    ...
    example:
      'ADDMONTH(项目开始时间,项目天数)，录入【项目开始时间】、【项目预计天数】，通过ADDMONTH函数自动填入【项目预计完成时间】',
    type: 'date',
  },
  {
    code: 'ADDYEAR()',
    ...
    example:
      'ADDYEAR(合同开始时间,合同有效年限)，录入【合同开始时间】、【合同有效年限】，通过ADDYEAR函数自动填入【合同到期时间】ADDMONTH(项目开始时间,项目天数)，录入【项目开始时间】、【项目预计天数】，通过ADDMONTH函数自动填入【项目预计完成时间】',
    type: 'date',
  },
  {
    code: 'TODAY()',
