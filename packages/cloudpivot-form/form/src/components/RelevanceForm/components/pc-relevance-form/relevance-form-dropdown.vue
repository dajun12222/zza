...
-->
<template>
  <div :id="refId" class="relevance-form-dropdown">
    <!-- 树形结构 -->
    <a-tree-select
      ...
      placeholder="请选择"
      allowClear
      :showSearch="control && control.options && control.options.showSearch"
...
export default class RelevanceFormDropdown extends RelevanceFormControl {
  ...
  @Watch('control.options', { deep: true, immediate: true })
  onChangeControl(newVal: any) {
    console.log(newVal);
  }
  ...
  popupScroll($ev) {
    if (
      ...
      this.page++;
      this.search(this.currentSeachValue, true, this.page);
    }
  }
  ...
  async search(value: string, searching: boolean = false, page?: number) {
    ...
    await this.fetch(
      ...
      (seg) => {
        ...
        this.loadedAll =
          seg.total === this.list.length || seg.data.length < 1000;
      },
      () => (this.fetching = false),
    );
  }
  ...
  created() {
    console.log('11111', this.selectedId);
    console.log('11111', this.ctrl);
    // dataProperties = JSON.parse(
    //   forDataProperty.bizSheet.publishedAttributesJson,
    // );
    // console.log('111',this.bizSheet
    // )

    // alert(111111)
    // listApi.serchName().then((res: any) => {
    //     if (res.errcode === 0) {
    //     }
    //   });
    ...
    if (this.treeView) {
      Bus.EventBus.$on(`setData_${this.control.key}`, (value) => {
        this.setValue(value);
      });
    }
    // this.setValue(this.ctrl.value.templateType);
    // this.selectedId=this.ctrl.value.templateType
  }
  ...
  mounted() {
    ...
    //把查询视图的关联单选需要的数据发送过去.
    this.treeViewSend();
    Bus.EventBus.$on(`${this.control.key}_search`, async (value) => {
      this.currentSeachValue = value;
      await that.search(value, true).then(() => {
        const loop = () => {
    ...
    Bus.EventBus.$on(
      `${this.control.key}_expends`,
      async (value, expandInfo) => {
        await that.onExpand(value).then(() => {
          const loop = () => {
            if (this.treeList.length) {
              that.sentEventBus();
            } else {
  ...
  async getNodeChildren(node) {
    ...
    if (
      ...
      queryData[1].push({
        propertyCode: this.displayFieldItem.code,
        propertyType: this.displayFieldItem.propertyType,
        propertyValue: this.currentSeachValue,
        queryFilterType: 'Like',
      });
    }
...
.relevance-form-dropdown {
  /deep/ .ant-select-selection__rendered .ant-select-selection__placeholder {
