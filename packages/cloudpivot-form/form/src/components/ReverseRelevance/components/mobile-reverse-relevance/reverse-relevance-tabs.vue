...
import SearchPanel from 'cloudpivot-form/form/src/components/RelevanceForm/components/mobile-relevance-form/search-panel.vue';
...
export default class ReverseRelevanceTabs extends ReverseRelevanceControl {
  ...
  superFetch(page: number, pageSize: number, value: string, formStatus: any) {
    ...
    params.forEach((param) => {
      param = param.map((res: any) => {
        const code =
          res.propertyCode !== undefined ? res.propertyCode : res.code;
        const type =
          res.propertyType !== undefined ? res.propertyType : res.type;
        const val =
