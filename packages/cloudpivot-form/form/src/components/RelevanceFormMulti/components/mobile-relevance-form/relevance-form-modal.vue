    ...
    >
      ...
      <template slot="fitler">
        <query-form
          ...
          :isRelevanceEntrance="isMobileRelevanceEntrance"
          @commit="onFilter"
        />
      </template>
    </search-panel>
...
export default class RelevanceFormModal extends RelevanceFormControl {
  ...
  change(item: any, isChecked: boolean) {
    ...
    if (Array.isArray(item)) {
      const selectedItems = this.value.items || this.value || [];
      const results = [...this.queryResult, ...selectedItems];
      item.forEach((x) => {
        let curItem = results.find((z) => {
          return z.id === x;
        });
        if (!curItem) {
          curItem = this.multiValue.find((z) => {
            return z.id === x;
          });
        }
  ...
  async confirm() {
    ...
    const item = await this.convertForMappings(
      this.multiValue.map((item) => item.data || item),
    );
