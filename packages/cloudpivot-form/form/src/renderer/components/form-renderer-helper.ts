...
export abstract class FormRendererHelper {
  ...
  static convertDesign(
    ...
    const reverseRelevanceArr = tabsList
      .filter((tab) => tab.options.visible)
      .map((c, index) => {
        return {
          code: index + 1,
          name: c.options.name,
    ...
    const groupFormControlList = [...GroupFormControlList];
  ...
  static getErrorMessage(
    ...
    switch (errCode) {
      case FormControlErrorCodes.Required:
        ...
        if (
          selectTypeControl.includes(control.type as schema.FormControlTypeMain)
        ) {
          return '请选择正确内容';
        }
        if (label === '数值') {
  ...
  static isSystem(
    type: schema.FormControlTypeMain | schema.FormControlTypeExtend,
  ) {
    return (
   ...
   * @param layout 表单设计器布局
   * @param controls 表单设计器控件对象
   * @param groupFormControlList 系统中存在的分组控件列表（按层级排布）
   * @param mapAndFilter 布局映射控件对象数组方法
   * @returns
   */
  static buildGroup(
    layout: string[][],
    controls: { [key: string]: FormControl },
    groupFormControlList: number[],
    mapAndFilter: (row: string[]) => schema.FormControl[],
  ) {
    const groupIndexs: number[] = [];
    const controlType: any = groupFormControlList.shift();
    layout.forEach((row, i) => {
      if (
        Array.isArray(row) &&
        row.length === 1 &&
        controls[row[0]] &&
        controls[row[0]].type === controlType
      ) {
        groupIndexs.push(i);
      }
    });
    ...
    if (groupIndexs.length > 0) {
      ...
      groupIndexs.forEach((idx, i) => {
        ...
        let children: RendererLayoutControl[] = [];
        if (!groupFormControlList.length) {
          children = layout
            .slice(idx + 1, groupIndexs[i + 1])
            .map(mapAndFilter)
            .map((row) => FormRendererHelper.buildRow(row));
        } else {
          const subLayout = layout.slice(idx + 1, groupIndexs[i + 1]);
          children = FormRendererHelper.buildGroup(
            subLayout,
            controls,
            [...groupFormControlList],
            mapAndFilter,
...
function getVerifyError(control: any) {
  ...
  return getVerifyErrorFn[
    control.type as
      | schema.FormControlTypeMain.Number
      | schema.FormControlTypeMain.Date
  ](control);
}
