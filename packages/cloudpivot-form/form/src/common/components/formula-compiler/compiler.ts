...
export class FormulaCompiler {
  ...
  constructor(options: GenerateParserOptions = {}) {
    ...
    const funcBuiltInNams = funcBuiltIn.map((el) => el.name);
    const hasNotFuncList = funcList.filter(
      (el) => !funcBuiltInNams.includes(el.name),
    );
    const add: any[] = [];
    hasNotFuncList.forEach((el) => {
      add.push({
        type: 'func',
        name: el.name,
        inputType: [
        ...
        outputType: {
          paramType: (el.returnType as string).toLowerCase(),
          sourceType: 'any',
        },
        ...
        validate(identifier, params) {
          return identifier.outputType as any;
        },
      });
    });

    this.setFuncs([...funcBuiltIn, ...funcs, ...add]);
  }
