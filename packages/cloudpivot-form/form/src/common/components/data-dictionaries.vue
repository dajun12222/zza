...
export default class DataLinkModal extends Vue {
  ...
  async searchDictionaryRecord() {
    this.spinning = true;
    const res: any = await dictionaryApi.getEnableRecordsByDictionaryCode({
      code: this.checkedDictionaryCode,
    });
    this.spinning = false;
    if (res.errcode === 0) {
  ...
  // 选中的数据字典改变
  checkedDictionaryChange(value) {
    this.plainOptions = [];
    this.checkedList = [];
    this.isCheckedDictionaryChange = true;
    this.checkedDictionaries = [];
    this.setCheckedDictionaryCode(value);
  }
  ...
  bizmodeBack(type?: boolean) {
    ...
    const obj = {
      ...
      checkedDictionaryCode: this.checkedDictionaryCode,
    ...
    if (this.modalData && this.modalData.value) {
      ...
      this.$nextTick(() => {
        if (value.useDictionariesData) {
          ...
          this.checkedDictionaryCode = value.checkedDictionaryCode;
          if (value.useDictionariesData) {
            this.checkedList = value.useDictionariesData.map((el) => el.id);
            this.oldCheckedList = JSON.stringify(this.checkedList); // 记录历史
            this.historyData = value.useDictionariesData;
  ...
  setCheckedDictionaryCode(id?: string) {
    const target = this.dictionaryList.find(
      (item) => item.id === id || item.id === this.checkedDictionary,
    );
    this.checkedDictionaryCode = target && target.code;
  }
}
</script>
