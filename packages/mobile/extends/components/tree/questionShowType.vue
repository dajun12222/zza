...
export default class questionShowType extends Vue {
    ...
    onChangeAuditTypeList(newVal:any){
        ...
        if((this.shorttext && this.shorttext.length > 0) && (newVal && newVal.length > 0)){
            ...
            this.$emit('input',auditList['dictionaryId']);
        }
    ...
    getParams(dictionaryId:string){
        let allArray:any = this.allGroup(JSON.parse(JSON.stringify(this.AuditTypeList)));
        let arr:any = this.allParent(dictionaryId,allArray);
        return arr || [];
    }

    allParent(dictionaryId:string,allArray:any){
        function getAllParent(dictionaryId:string,allArray:any,att:any = []){
            if(dictionaryId && (allArray && allArray.length > 0)){
                let currentObj:any = allArray.find((item:any) => {
                    return item['dictionaryId'] === dictionaryId;
                });
        ...
        return getAllParent(dictionaryId,allArray,[]) || [];
    }
