...
<!-- <a-modal
    ...
    <!-- <div class="auditModal" :class="{ 'auditModal1': getContailLayout,'activeAuditModal': isMediation }"> -->
        <!-- <div v-if="mode !== 'showData'" class="importData">
...
export default class auditModal extends Vue {
    ...
    async review(item:any){
        this.reviewObj = item;
        const res: any = await api.similarBasis({qualitativeProblem: item.qualitativeProblem,qualitativeBasis: item.qualitativeBasis});        
        if (res.data) {
    ...
    get getContailLayout(){
        ...
        if(!this.isMediation && this.mode !== 'review' && this.mode !== 'showhtml'){
            status = false;
        }
        return status;
    }
