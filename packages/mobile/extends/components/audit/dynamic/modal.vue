<template>
    <a-modal :visible="show" title="详情" class="indexModal" @cancel="close" @ok="close">
        <table class="modalDiv">
            <tbody>
                <tr v-for="(item,index) in modalData" :key="index">
                    <template v-for="(items,i) in item">
                        <th :key="'index' + i">{{ items.name }}</th>
                        <td v-if="items.name === '动态附件' && items.con" :key="'i' + i" :colspan="items.cols">
                          <div v-html="items.con"></div>
                            <span class="action">
                                <i class="icon aufontAll h-icon-all-download" title="下载" @click="download(items.con)"></i>
                            </span>
                        </td>
                        <td v-else :key="'i' + i" :colspan="items.cols"><div v-html="items.con"></div></td>
                    </template>
                </tr>
            </tbody>
        </table>
