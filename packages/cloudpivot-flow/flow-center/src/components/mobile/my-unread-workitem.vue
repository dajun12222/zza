...
export default class Circulates extends Vue {
  ...
  get selectedNum() {
    let arr: any[] = [];
    arr = this.checkAll
      ? JSON.parse(JSON.stringify(this.circulates))
      : JSON.parse(JSON.stringify(this.checkedIds));
    if (this.checkAll && this.checkedIds.length > 0) {
      arr = [];
      for (const con of this.circulates) {
        const ids = this.checkedIds.find((x) => x === con.id);
        if (!ids) {
          arr.push(con.id);
        }
      }
    ...
    if (arr.length === 0) {
      this.$store.commit('circulate/resetCheckCheckAll');
    }
    return arr.length;
  }
  ...
  markMultiRead() {
    ...
    let params: string[] = this.checkAll ? [] : this.checkedIds;
    if (this.checkAll && this.checkedIds.length > 0) {
      const arr: any = [];
      for (const con of this.circulates) {
        const ids = this.checkedIds.find((x) => x === con.id);
