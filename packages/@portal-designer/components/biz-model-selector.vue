...
export default class bizModelSelector extends Vue {
  groupCtrlValue: string = '';

  @Prop()
  value!: string;

  @Prop({ default: false })
  onlyBizModel!: boolean;

  onlyPublish: boolean = true;
      ...
      function formatTree(list, ctx) {
        if (list && list.length > 0) {
          return list
            ...
            .filter((childnode) =>
              that.onlyBizModel
                ? !['Page', 'Report'].includes(childnode.type)
                : childnode.type,
            );
        }
      }
  ...
  onChange(val: string, vNode: any) {
    ...
    if (!val) {
      ...
      this.$emit('emitChange', theModel.value);
      this.$emit('input', theModel.value);
   ...
   */
  async setDefault() {
    if (this.groupCtrlValue !== this.lastDefault) {
      this.lastDefault = this.groupCtrlValue;
    } else {
      return;
    }
    const res: any = await listApi.getAppCodeByModelCode({
      schemaCode: this.groupCtrlValue as any,
    });
    if (res.errcode === 0) {
      ...
      this.onLoadData(treeNode, appList).then((res: any) => {
        if (res) {
          ...
          if (this.selectModel !== '') {
            ...
            const theModel: any = filterTableMater(
              this.groupCtrlValue as any,
              this.treeData,
            );
            this.result = theModel ? [theModel.key] : [];
          } else {
  ...
  created() {
    this.groupCtrlValue = this.value;
  ...
  @Watch('groupCtrlValue', { immediate: true })
  onValueChange() {
    if (!this.groupCtrlValue as any) {
      this.result = [];
      this.selectModel = '';
    ...
    this.selectModel = this.groupCtrlValue as any;
    this.setDefault();
  }
}
</script>
