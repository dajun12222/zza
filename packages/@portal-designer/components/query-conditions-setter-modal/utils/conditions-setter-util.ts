...
若有问题，请参考前端相关二开文档。
*/
import * as ControlFactory from 'cloudpivot-list/list/src/components/pc/helper/control-factory';
...
export function toFormControl(field: any) {
  if (!field.filterType) {
    field.filterType = getDefaultFilterType(field);
  }
  if ([12, 14].includes(field.propertyType)) {
    field.displayType = 3;
  }
...
function getDefaultFilterType(control) {
  switch (control.propertyType) {
    case renderer.DataItemType.Date:
    case renderer.DataItemType.Number:
    case renderer.DataItemType.Time:
      return 'Between';
    case renderer.DataItemType.ShortText:
    case renderer.DataItemType.Checkbox:
    case renderer.DataItemType.DropdownMulti:
    case renderer.DataItemType.Address:
    case renderer.DataItemType.StaffMulti:
    case renderer.DataItemType.DeptMulti:
    case renderer.DataItemType.StaffDeptMix:
    ...
    case renderer.DataItemType.StaffSingle:
    case renderer.DataItemType.DeptSingle:
    case renderer.DataItemType.Radio:
    case renderer.DataItemType.Dropdown:
    case renderer.DataItemType.Logic:
      return 'Eq';
    default:
      return 'isNull';
