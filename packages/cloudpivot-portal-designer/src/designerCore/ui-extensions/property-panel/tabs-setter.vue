              ...
              <template slot="content">
                <div class="img-setting-box">
                  ...
                  <div class="form-item">
                    ...
                    >
                      <a-select-option value="bizModel">业务模型</a-select-option>
                      <a-select-option value="myUnfinished">我的待办</a-select-option>
                      <a-select-option value="myFinished">我的已办</a-select-option>
                      <a-select-option value="myRead">我的已阅</a-select-option>
                      <a-select-option value="myUnread">我的待阅</a-select-option>
                      <a-select-option value="myStart">我发起的</a-select-option>
                    </a-select>
                  </div>

                  <div
                    v-if="item.dataSource && item.dataSource === 'bizModel'"
                    class="form-item biz-model"
                  >
                    <bizModelSelector
                      v-model="item.bizModel"
                      :onlyBizModel="true"
                      @emitChange="
                        (value) => {
                          item.displayField = '';
                          item.sortField = '';
                          item.showFieldTitle = false;
                  ...
                  <div v-if="item.dataSource" class="form-item">
                    <div class="label">展示字段</div>
                    <DisplayFieldSelector
                      :fieldList="
                        item.displayField ? JSON.parse(item.displayField) : []
                      "
                      :dataSource="item.dataSource"
                      :bizModel="item.bizModel"
                      :ignoreDateItem="displayFieldIgnoreItem"
                      @emitChange="
                        (value) => {
                          item.displayField = JSON.stringify(value);
                        }
                      "
                    />
                  </div>
                  ...
                  <div v-if="item.dataSource" class="form-item">
                    <div class="label">显示字段标题</div>
                    <a-switch v-model="item.showFieldTitle" size="small" />
                  </div>
                  ...
                  >
                    ...
                    <QueryConditionsSetter
                      :value="item.queryConditions"
                      :dataSource="item.dataSource"
                      :bizModel="item.bizModel"
                      @valueChange="
                        (value) => {
                          item.queryConditions = value;
                        }
                      "
                    />
                  </div>
                  ...
                  <div v-if="item.dataSource" class="form-item">
                    ...
                    <FieldSelector
                      :field="item.sortField ? JSON.parse(item.sortField) : ''"
                      :dataSource="item.dataSource"
                      :bizModel="item.bizModel"
                      :ignoreDataItem="sortFieldIgnoreItem"
                      :fieldList="
                        item.displayField ? JSON.parse(item.displayField) : []
                      "
                      :disabled="
                        item.dataSource && item.dataSource !== 'bizModel'
                      "
                      :fieldDataSource="
                        item.displayField ? JSON.parse(item.displayField) : []
                      "
                      @emitChange="
                        (value) => {
                          item.sortField = JSON.stringify(value);
                        }
                      "
                    />
                  </div>
                  ...
                  <div v-if="item.dataSource" class="form-item">
                    ...
                    <a-radio-group
                      ...
                      "
                    >
                      <label><a-radio value="ascending">升序</a-radio></label>
                      <label><a-radio value="descending">降序</a-radio></label>
                    </a-radio-group>
                  </div>

                  <div v-if="item.dataSource" class="form-item set-list-symbol">
                    <div class="label">列表符号</div>
                    <a-radio-group v-model="item.listIcon">
                      <template v-for="(groupItem, groupIndex) in groupOptions">
                        <a-radio
                          :key="groupIndex"
                          :value="groupItem.value"
                          :class="groupItem.value"
                          >{{ groupItem.name }}</a-radio>
                      </template>
                    </a-radio-group>
                  </div>
                </div>
...
export default class InputUploadProperty extends Mixins(PropertyComponent) {
  ...
  //显示字段过滤的数据项类型
  displayFieldIgnoreItem: DataItemType[] = [
  ...
  //排序字段过滤的数据项类型
  sortFieldIgnoreItem: DataItemType[] = [
    DataItemType.Attachment,
    DataItemType.Sheet,
    DataItemType.RelevanceForm,
    DataItemType.RelevanceFormEx,
  ...
  onDataSourceChange(item) {
    ...
    item.showFieldTitle = false;
    item.queryConditions = '';
    if (item.dataSource !== 'bizModel') {
      //非数据模型的数据源，排序字段始终为接受时间
      item.sortField =
        '{"name":"接收时间","name_i18n":null,"type":3,"value":"receiveTime"}';
      item.sortRule = 'descending';
    } else {
      item.sortField = '';
      item.sortRule = 'ascending';
    }
  }
