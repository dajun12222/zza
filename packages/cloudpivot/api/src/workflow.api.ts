...
export class WorkflowApi {
  ...
  deleteInstance(
    ...
    return axios.post(
      `${Mappings.workflow.deleteInstance}?workflowInstanceId=${workflowInstanceId}&workItemId=${workItemId}&version=${workflowVersion}`,
    );
  }
  ...
  deleteWorkflowAdmin(params: any) {
    ...
    return axios.post(
      `/api/workflow/delete_workflow_admin?${Params.join('&')}`,
    );
  }
  ...
  //获取某条待办流程的最后一条催办信息
  getLastUrgeInfo(params: any) {
    return axios.post(`/api/runtime/workflow/get_urge_info`, params);
  }

  //获取协办任务列表
  getAssistInfo(params: any) {
    return axios.get('/api/runtime/workflow/get_assist_info', {
      params,
    });
  }

  //撤回所有协办任务
  revokeAssistWorkitem(params: any) {
    return axios.get('/api/runtime/workflow/revoke_assist_workitem', {
      params,
    });
  }

  // 流程审批记录中预测后续节点
  workflowPredict(params: any) {
    return axios.post(Mappings.workflow.workflowPredict, params);
  }
}
