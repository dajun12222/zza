...
export const dateFormatApm = (date: Date, fmt: string) => {
  const o: any = {
    ...
    'D+': date.getDate(), // 日
    'H+':
      fmt.includes('APM') && date.getHours() > 12
        ? date.getHours() - 12
        : date.getHours(), // 小时
    'm+': date.getMinutes(), // 分
    's+': date.getSeconds(), // 秒
...
export const msesToTimeStr = (msec: number) => {
  ...
  if (!!dateObj) {
    if (dateObj.day !== 0) {
      dateStr += `${isZH ? `${dateObj.day}天` : `${dateObj.day} day `}`;
    }
    if (dateObj.hour !== 0) {
      dateStr += `${isZH ? `${dateObj.hour}小时` : `${dateObj.hour} hr `}`;
    }
    if (dateObj.minute !== 0) {
      dateStr += `${isZH ? `${dateObj.minute}分钟` : `${dateObj.minute} min `}`;
    }
    if (dateObj.second !== 0) {
      dateStr += `${isZH ? `${dateObj.second}秒` : `${dateObj.second} sec `}`;
    }
  }
  return dateStr;
};
...
export const timeTranslate = (time: number) => {
  ...
  if (time === 0) {
    return isZH ? '1分钟' : '1min';
  }

  let _day: string = `${
    isZH ? `${Math.abs(_t.day)}天` : `${Math.abs(_t.day)}day `
  }`;
  if (_t.day === 0) {
    _day = '';
  }
  ...
  let _hour: string = `${
    isZH ? `${Math.abs(_t.hour)}小时` : `${Math.abs(_t.hour)}hr `
  }`;
  ...
  let _minute = `${
    isZH ? `${Math.abs(_t.minute)}分钟` : `${Math.abs(_t.minute)}min`
  }`;
  if (_t.minute === 0) {
